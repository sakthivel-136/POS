...
def get_customer_pending_balance(customer_id: int, supabase: Client = Depends(get_supabase)):
    res = supabase.table('customers').select('credit_limit').eq('id', customer_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="Customer not found")
    
    total_pending = float(res.data[0].get('credit_limit') or 0.0)
...
def update_bill(bill_id: int, bill_update: schemas.BillUpdate, supabase: Client = Depends(get_supabase)):
    ...
    # FIFO Payment Allocation for excess payment
    # To do this safely in simple update, we need the total_amount from the old_bill
    total_amount = float(old_bill.get('total_amount', 0))
    excess_payment = float(bill_update.paid_amount) - total_amount
